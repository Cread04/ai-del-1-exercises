...
AI är det bredaste begreppet och handlar om själva tekniken som gör det möjligt för maskiner att kunna efterlikna männsklig intelligens som t.ex logik och planering eller kreativitet.

Maskininlärning är en delmängd av AI det är metoder där datorer lär sig mönster från data utan att vara explicit programmerade för varje specifik regel.

Djupinlärning är en delmängd av maskininlärning och det använder Artificiella neurala närverk med många lager för att lösa komplexa problem som då bildigenkänning osv..

Man skulle kunna säga aatt de är som ryska dockor AI är helheten sedan är ML en delmmängd av AI och DL är en delmmängd av ML. 



2. Vilka är de fyra problemkategorierna inom ML? 

1. Supervised learning: Datan har facit tex Klassificering eller regression

2. unsupervised Learning: Datan har ingen facit och modellen letar strukturer själv som t.ex klusring

3. Semi-supervised Learning: det är en blanding med data, facit och mycket utan.

4. Reinforcement Learning: En agent som lär sog genom att intergera med en miljö och får belöningar eller straff.


3. Förklara följande: 
a) Vad är syftet med att dela upp data i träningsdata, valideringsdata och testdata? 

för att modellen ska kunna bli så effektiv och bra som möjligt måste vi dela upp det 

tränings data: för att modellen ska lära sig parametrarna

valideringsdata: används under träningsprocessen för att ställa in hyperparameterar och välja den modell som är bäst. 

Testdata: Används endast en gång på slutet för att kunna bedöma hur modellen presterar på helt ny data.

b) Vad är k-delad korsvalidering för något? 

 k-delad korsvalidering är när modellenb tränas K gånger varje gång används k - 1 delar för träning och 1 del för validering 
 slutresultatet är genomsnittet av dessa omgångarna och detta ger en säkrare bedömning av modellens prestanda särkligt vid mindre dataset.


c) Vad är *root mean squared error* (*RMSE*) för något?
 RMSE är ett mått på felet i regressionsmodeller och man vill såklart ha så lite fel som möjligt. ekvationen är roten ur medelvärdet av de kvaderade felen.


d) Vad är en hyperparameter för något? Vad är en parameter för något? 

Hyperparameter: är inställningar som vi bestämmer innan träningen börja alltå t.ex learning rate. 

parameter: är inre variabler som modellen lär sig själv från datan tex lutningen i en linjär regression.
...
Grid Search är när man definerar ett rutnät av hyperparamterar man vill testa

processen: algoritmen söker igenom alla kombinationer i rutnätet för att hitta den som ger bäst resultat.

refit=true: är när den bästa kombinationen av hyperaparamter är hittad så tränar scikit learn automatiskt 
om modellen en sista gång på hela datasetet så alltså (tränings + valideringsdatan i korsvaldieringen) med dessa inställningar. 
Det gör att vi får en modell tränad på maximal mängd data 
